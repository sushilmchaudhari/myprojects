...
case $SERVER_TYPE in
    ...
    backend|BACKEND)
        if [ $INSTANCE_ENV == "prod" ]; then
            SUBNET_ID="subnet-5e2b3f06"
            SG_NAME="nextgen_prod_backend_security_group"
        elif [ "$INSTANCE_ENV" == "staging" ]; then
            SUBNET_ID="subnet-57884e30"
...
app_name = "$INSTANCE_ENV-$SERVER_TYPE"
EOL

# Run Terraform Script to create Instances.
/Users/test/terraform/terraform apply ../instances/
