...
resource "aws_eip" "pub_ip_fixed" {
  ...
  vpc = true
  tags = "${merge(map("Name", format("%s-${local.envi}-%s-eip", var.name, var.ec2_suffix)), var.tags)}"
}
...
resource "aws_instance" "server" {
  ...
  tags = "${merge(map("Name", format("%s-%s-%s-ec2-%s-%d", var.name, local.envi, var.ec2_suffix, element(var.az_ids, count.index) ,count.index+1)), var.tags)}"
  volume_tags = "${merge(map("Name", format("%s-%s-%s-root-%s-%d", var.name, local.envi, var.ec2_suffix, element(var.az_ids, count.index) ,count.index+1)), var.tags)}"
...
resource "aws_ebs_volume" "data" {
  ...
  tags              = "${merge(map("Name", format("%s-%s-%s-data-%s-%d", var.name, local.envi, var.ec2_suffix, element(var.az_ids, count.index) ,count.index+1)), var.tags)}"
}
