...
resource "aws_db_parameter_group" "pgroup" {
  ...
  parameter = ["${var.pgroup_parameters}"]

  tags = "${merge(var.tags, map("Name", format("%s-%s-rds-paramerter-group", var.name, local.envi)))}"
...
resource "aws_db_instance" "db" {
  ...
  parameter_group_name   = "${aws_db_parameter_group.pgroup.id}"
  option_group_name      = "${element(concat(aws_db_option_group.ogroup.*.id, list("")), 0)}"
  ...
  tags = "${merge(var.tags, map("Name", format("%s-%s-db", var.db_identifier, local.envi)))}"
}
