...
resource "aws_lb" "application" {
  ...
  tags                             = "${merge(var.tags, map("Name", format("%s-%s-alb", var.name, local.envi)))}"
...
resource "aws_lb_target_group" "http_target_group" {
  ...
  tags       = "${merge(var.tags, map("Name", lookup(var.http_target_group, "name")))}"
...
resource "aws_lb_target_group" "https_target_group" {
  ...
  tags       = "${merge(var.tags, map("Name", lookup(var.https_target_group, "name")))}"
  depends_on = ["aws_lb.application"]
...
resource "aws_lb_listener" "frontend_https" {
  ...
  default_action {
    target_group_arn = "${aws_lb_target_group.https_target_group.id}"
    type             = "forward"
  }
}
