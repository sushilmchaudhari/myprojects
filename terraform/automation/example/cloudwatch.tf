...
resource "aws_sns_topic" "alarm" {
  name = "${format("%s-%s-cw-alarm-topic", var.name, lookup(var.tags, "Environment"),)}"
}
...
resource "aws_cloudwatch_metric_alarm" "cpu-utilization" {
  ...
  alarm_name          = "cpu-utilization-${element(concat(module.app-ec2.instance_ids_cloudwatch,module.worker-ec2.instance_ids_cloudwatch), count.index)}"
...
resource "aws_cloudwatch_metric_alarm" "memory-utilization" {
  count               = "${var.enable_cloudwatch ? var.app_instance_count + var.worker_instance_count : 0}"
  alarm_name          = "memory-utilization-${element(concat(module.app-ec2.instance_ids_cloudwatch,module.worker-ec2.instance_ids_cloudwatch), count.index)}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
...
resource "aws_cloudwatch_metric_alarm" "instance-health-check" {
  count               = "${var.enable_cloudwatch ? var.app_instance_count + var.worker_instance_count : 0}"
  alarm_name          = "instance-health-check-${element(concat(module.app-ec2.instance_ids_cloudwatch,module.worker-ec2.instance_ids_cloudwatch), count.index)}"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "2"
...
resource "aws_cloudwatch_metric_alarm" "burst_balance_too_low" {
  ...
  dimensions {
    DBInstanceIdentifier = "${module.master_db.db_instance_id}"
  }
}
...
resource "aws_cloudwatch_metric_alarm" "redis_cache_cpu_util" {
  ...
  alarm_name          = "cpu-utilization-${element(flatten(module.ec-redis-cache.elasticache_member_clusters), count.index)}"
  ...
  dimensions {
    CacheClusterId = "${element(flatten(module.ec-redis-cache.elasticache_member_clusters), count.index)}"
  }
}

resource "aws_cloudwatch_metric_alarm" "redis_jobs_cpu_util" {
  count               = "${var.enable_cloudwatch ? var.redis_jobs_node_count : 0}"
  alarm_name          = "cpu-utilization-${element(flatten(module.ec-redis-jobs.elasticache_member_clusters), count.index)}"
  ...
  dimensions {
    CacheClusterId = "${element(flatten(module.ec-redis-jobs.elasticache_member_clusters), count.index)}"
  }
}

resource "aws_cloudwatch_metric_alarm" "redis_cache_swap_util" {
  count               = "${var.enable_cloudwatch ? var.redis_cache_node_count : 0}"
  alarm_name          = "swap-utilization-${element(flatten(module.ec-redis-cache.elasticache_member_clusters), count.index)}"
...
resource "aws_cloudwatch_metric_alarm" "redis_jobs_swap_util" {
  count               = "${var.enable_cloudwatch ? var.redis_jobs_node_count : 0}"
  alarm_name          = "swap-utilization-${element(flatten(module.ec-redis-jobs.elasticache_member_clusters), count.index)}"
